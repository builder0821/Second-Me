...
&emsp;文字：原始文本 -> 分词器Tokenizers -> TokenID --> 文本Embedding向量表 ---> X -> Transformer浮点矩阵+外挂LoRA  
&emsp;音频：原始波形 -> 梅尔频谱二维提炼 -> H/W编码 --> 音频Embedding向量表 -> X -> Transformer浮点矩阵+外挂LoRA  
&emsp;图片：原图归一缩放 -> ViT切分Patch块 -> 单帧图像Patch向量 ------------------> X -> Transformer浮点矩阵+外挂LoRA  
&emsp;视频：隔帧抽取缩放 -> ViT切分Patch块 -> 多帧Patch向量融合 -> 时序向量 ----> X -> Transformer浮点矩阵+外挂LoRA  
